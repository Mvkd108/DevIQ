...
export class JiraPicker {
    ...
    public async ensureIssuesLoaded(): Promise<void> {
    ...
    public getActiveIssue(): JiraIssue | null {
        if (!this.activeIssueId) {
            return null;
        }

        return this.issues.find(issue => issue.issue_id === this.activeIssueId) ?? {
            issue_id: this.activeIssueId,
