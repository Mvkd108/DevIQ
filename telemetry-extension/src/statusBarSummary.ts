...
import { UploadBoundaryPreview, TrackingControls } from './types';
...
export class StatusBarSummary implements vscode.Disposable {
    ...
    public refresh(): void {
        ...
        const showStatusBarSummary = config.get<boolean>('showStatusBarSummary', true);
        ...
        const uploadConfigured = Boolean(
            config.get<string>('supabaseUrl', '') && config.get<string>('supabaseKey', '')
        );
    ...
    ): { label: string; text: string; backgroundColor?: vscode.ThemeColor } {
        if (!telemetryEnabled) {
            return {
                label: 'Local only',
    ...
    private describeSensitiveSignals(presenceEnabled: boolean, backgroundEnabled: boolean): string {
        const enabled = [
    ...
    private inlinePreview(items: string[]): string {
        if (items.length === 0) {
            return 'none';
        }
