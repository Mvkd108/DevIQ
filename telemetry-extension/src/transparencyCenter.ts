...
import { NextCommitPreview, TrackingControls, TransparencyStatusItem, UploadBoundaryPreview } from './types';

export class TransparencyCenter {
    constructor(
        private readonly aggregator: SignalAggregator,
        private readonly activityMonitor: ActivityMonitor,
        private readonly cameraMonitor: CameraMonitor,
        private readonly backgroundMonitor: BackgroundMonitor,
        private readonly jiraPicker: JiraPicker,
        private readonly gitListener: GitListener
    ) {}

    public async showTransparencyOverview(): Promise<void> {
    ...
    public async showSessionSummary(): Promise<void> {
    ...
    public async showDataCollectionStatus(): Promise<void> {
        ...
        const selection = await vscode.window.showQuickPick(
            this.buildDataCollectionItems(status, controls, uploadBoundary),
            {
    ...
    public async showActiveIssue(): Promise<void> {
        await this.jiraPicker.ensureIssuesLoaded();
        const activeIssue = this.jiraPicker.getActiveIssue();

        if (!activeIssue) {
            const selection = await vscode.window.showInformationMessage(
                'DevHouse: no active Jira issue is selected. Commits still stay local or upload normally, but the next commit will not be linked to Jira until you choose one.',
                'Select Issue',
                'Show Data Status'
            );
        ...
        const selection = await vscode.window.showInformationMessage(
            `DevHouse issue: ${activeIssue.issue_id} (${activeIssue.status}) - ${activeIssue.title}.${description} It is only attached when a commit is prepared; nothing uploads continuously.`,
            'Change Issue',
            'Show Data Status'
        );
    ...
    public async manageTrackingSettings(): Promise<void> {
        const controls = this.getTrackingControls();
    ...
    public async resetSessionState(): Promise<void> {
        const selection = await vscode.window.showWarningMessage(
            'Reset local DevHouse session metrics, presence summaries, and background monitoring state?',
            { modal: true },
            'Reset Session'
        );
        ...
        logger.appendLine('[SESSION] Local session state was reset by the developer.');
    ...
    private getTrackingControls(): TrackingControls {
        const config = vscode.workspace.getConfiguration('devintel');
        ...
        return {
            ...
            backgroundMonitoringEnabled: config.get<boolean>('backgroundMonitoringEnabled', true),
            uploadConfigured: Boolean(supabaseUrl && supabaseKey),
            statusBarSummaryEnabled: config.get<boolean>('showStatusBarSummary', true)
        };
    }

    private async buildTransparencyMarkdown(): Promise<string> {
        const controls = this.getTrackingControls();
        await this.jiraPicker.ensureIssuesLoaded();
        ...
        lines.push(
            ...
            ...this.renderBoundarySection(uploadBoundary.localOnly),
            '',
            '- Uploaded on commit when telemetry upload is enabled and configured:',
            ...this.renderBoundarySection(uploadBoundary.uploadedOnCommit),
            '',
            '- Optional when enabled and excluded when disabled:',
            ...this.renderBoundarySection(uploadBoundary.optionalWhenEnabled),
            '',
            '## Never Uploaded',
            '',
            ...this.renderBoundarySection(uploadBoundary.neverUploaded),
            '',
            '## Useful Commands',
            '',
            '- `DevHouse: Open Transparency Center`',
            '- `DevHouse: Show Session Summary`',
            '- `DevHouse: Show Data Collection Status`',
            '- `DevHouse: Show Active Issue`',
            '- `DevHouse: Manage Tracking Settings`',
            '- `DevHouse: Open Settings`',
            '- `DevHouse: Reset Session State`'
        );

        return lines.join('\n');
    }

    private renderNextCommitPreview(
        preview: NextCommitPreview,
        controls: TrackingControls,
        session: ReturnType<SignalAggregator['getSession']>,
        activeIssueId: string | null,
        presence: ReturnType<CameraMonitor['getStatus']>,
        background: ReturnType<BackgroundMonitor['getStatus']>
    ): string[] {
        const uploadReady = controls.telemetryEnabled && controls.uploadConfigured;
    ...
    private async buildQuickStatusMessage(): Promise<string> {
        const controls = this.getTrackingControls();
        await this.jiraPicker.ensureIssuesLoaded();
        const session = this.aggregator.getSession();
        const activeIssue = this.jiraPicker.getActiveIssue();
    ...
    private async buildSessionSummaryMessage(): Promise<string> {
        const controls = this.getTrackingControls();
        const session = this.aggregator.getSession();
    ...
    private buildDataCollectionItems(
        status: string,
        controls: TrackingControls,
        uploadBoundary: UploadBoundaryPreview
    ): TransparencyStatusItem[] {
        const optionalSignals = [
            controls.presenceEnabled ? 'presence summaries' : null,
            controls.backgroundMonitoringEnabled ? 'background summaries' : null
        ].filter(Boolean);
    ...
    private renderBoundarySection(items: string[]): string[] {
        if (items.length === 0) {
    ...
    private getSessionStart(sessionId: string): Date | null {
    ...
    private getSessionAgeMinutes(sessionStart: Date | null): number {
        if (!sessionStart) {
            return 0;
        }
    ...
    private formatDurationFromMinutes(totalMinutes: number): string {
    ...
    private inlineList(items: string[], fallback: string): string {
        if (items.length === 0) {
            return fallback;
        }
    ...
    private toggleIcon(enabled: boolean): string {
