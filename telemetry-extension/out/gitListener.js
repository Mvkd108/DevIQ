...
class GitListener {
    ...
    config;
    repositories = [];
    warnedRepositoryIdentity = new Set();
    constructor(aggregator, activityMonitor, webhookSender, cameraMonitor, backgroundMonitor, jiraPicker) {
        ...
        extension_1.logger.appendLine('[GIT] DevHouse commit monitoring ready.');
    }
    updateConfig() {
        const config = vscode.workspace.getConfiguration('devintel');
        let developerId = (config.get('developerId', '') || '').trim();
        ...
        const repositoryFallback = (config.get('repositoryName', '') || '').trim() || 'repository';
        this.config = {
    ...
    async initialize() {
        ...
        if (!gitExtension) {
            extension_1.logger.appendLine('[GIT] VS Code Git support is unavailable. Commit monitoring is paused.');
            return;
        }
        ...
        if (!gitAPI) {
            extension_1.logger.appendLine('[GIT] VS Code Git API could not be loaded. Commit monitoring is paused.');
            return;
        }
        const setupRepo = async (repo) => {
            const repoPath = repo.rootUri.fsPath;
            extension_1.logger.appendLine(`[GIT] Tracking repository: ${repoPath}`);
            ...
            this.backfillHistoricalCommits(repoPath, repo).catch(err => {
                extension_1.logger.appendLine(`[SYNC] Historical sync could not finish for ${path.basename(repoPath)}: ${err}`);
            });
    ...
    ensureDotDevPulseFolders(repoPath) {
        ...
        if (!fs.existsSync(devpulseGitignore)) {
            ...
            extension_1.logger.appendLine('[LOCAL] Created a private `.devpulse` mirror for local commit snapshots.');
        }
        ...
        try {
            if (fs.existsSync(rootGitignore)) {
                ...
                if (!lines.some(line => line.trim() === '.devpulse' || line.trim() === '.devpulse/')) {
                    ...
                    extension_1.logger.appendLine('[LOCAL] Added `.devpulse/` to the repository `.gitignore`.');
                }
            }
            else {
                fs.writeFileSync(rootGitignore, '.devpulse/\n', 'utf8');
                extension_1.logger.appendLine('[LOCAL] Created a repository `.gitignore` entry for `.devpulse/`.');
            }
        }
        catch (err) {
            extension_1.logger.appendLine(`[LOCAL] Could not update the repository \`.gitignore\`: ${err}`);
        }
    }
    async backfillHistoricalCommits(repoPath, repo) {
        ...
        try {
            ...
            extension_1.logger.appendLine(`[SYNC] Found ${unsyncedHashes.length} earlier commit snapshot(s) to mirror locally or retry for upload.`);
            let syncedCount = 0;
            ...
            for (const hash of unsyncedHashes.reverse()) {
                try {
                    ...
                    extension_1.logger.appendLine(`[SYNC] Could not process historical commit ${hash.substring(0, 7)}: ${err}`);
                }
            }
            extension_1.logger.appendLine(`[SYNC] Historical sync complete: ${syncedCount}/${unsyncedHashes.length} earlier commit snapshot(s) uploaded.`);
            ...
            extension_1.logger.appendLine(`[SYNC] Historical sync scan failed for ${path.basename(repoPath)}: ${err}`);
        }
    }
    async syncExistingEvents(repoPath) {
        ...
        try {
            ...
            if (pendingFiles.length === 0)
                return;
            extension_1.logger.appendLine(`[SYNC] Retrying ${pendingFiles.length} locally saved commit snapshot(s).`);
            for (const file of pendingFiles) {
                ...
                try {
                    ...
                    if (success) {
                        ...
                        if (moved) {
                            extension_1.logger.appendLine(`[SYNC] Uploaded locally saved snapshot ${file}.`);
                        }
                    }
                }
                catch (err) {
                    extension_1.logger.appendLine(`[SYNC] Could not upload local event ${file}: ${err}`);
                }
            }
            await this.reconcileSupabaseWithLocalMirror(repoPath);
        }
        catch (err) {
            extension_1.logger.appendLine(`[SYNC] Could not inspect local event storage: ${err}`);
        }
    }
    async checkForNewCommit(repo) {
        ...
        if (previousCommitId !== currentCommitId) {
            extension_1.logger.appendLine(`[GIT] New commit detected: ${currentCommitId.substring(0, 7)}.`);
            ...
            try {
                ...
                if (stats.files.length === 0 && stats.additions === 0 && stats.deletions === 0) {
                    extension_1.logger.appendLine(`[GIT] Commit ${currentCommitId.substring(0, 7)} had no tracked file changes.`);
                    return;
                }
                ...
                if (success && filePath) {
                    ...
                    extension_1.logger.appendLine('[SYNC] Upload did not complete. The local commit snapshot was kept and will be retried later.');
                }
                ...
                extension_1.logger.appendLine('[SESSION] Local session metrics were cleared after the commit event was processed.');
            }
            catch (error) {
                extension_1.logger.appendLine(`[GIT] Could not process the new commit event: ${error}`);
            }
        }
    }
    async getCommitStats(commitId, repoPath) {
        ...
        try {
            ...
            if (!repositoryOwner && !this.warnedRepositoryIdentity.has(repoPath)) {
                ...
                extension_1.logger.appendLine(`[GIT] No remote repository owner was detected for ${path.basename(repoPath)}. DevHouse will use the local repository name for commit events.`);
            }
            ...
            extension_1.logger.appendLine(`[GIT] Git command failed while building a commit event: ${err}`);
            throw err;
        }
    }
    mapGitStatus(status) {
    ...
    async saveEventLocally(event, repoPath) {
        try {
            ...
            extension_1.logger.appendLine(`[LOCAL] Saved local-only commit snapshot ${event.commit_id.substring(0, 7)}.`);
            return filePath;
        }
        catch (err) {
            extension_1.logger.appendLine(`[LOCAL] Could not save a local commit snapshot: ${err}`);
            return undefined;
        }
    }
    moveToSynced(filePath) {
        try {
            ...
            extension_1.logger.appendLine(`[SYNC] Could not move a local event into the synced mirror: ${err}`);
            return false;
        }
    }
    ...
    loadEventFileMap(directory) {
        ...
        for (const file of fs.readdirSync(directory)) {
            ...
            try {
                ...
                extension_1.logger.appendLine(`[SYNC] Could not read local event ${filePath}: ${error}`);
            }
        }
        return events;
    }
    collectLocalEventMirror(repoPath) {
        const { eventsDir, syncedDir } = this.getEventStoragePaths(repoPath);
    ...
    async getNextCommitPreview() {
        const previews = await Promise.all(this.repositories.map(async (repo) => {
            const repoPath = repo.rootUri.fsPath;
    ...
    async reconcileSupabaseWithLocalMirror(repoPath) {
        const localEvents = this.collectLocalEventMirror(repoPath);
        const expectedCommitIds = new Set(localEvents.keys());
        extension_1.logger.appendLine(`[SYNC] Reconciling ${expectedCommitIds.size} local commit event(s) with the remote mirror.`);
        for (const event of localEvents.values()) {
            const success = await this.webhookSender.sendToSupabase(event);
            if (!success) {
                extension_1.logger.appendLine(`[SYNC] Could not mirror local event ${event.commit_id} to the remote store.`);
            }
        }
        const repositoryName = path.basename(repoPath) || this.config.repositoryName;
        ...
        for (const commitId of remoteCommitIds) {
            if (!expectedCommitIds.has(commitId)) {
                ...
                if (deleted) {
                    extension_1.logger.appendLine(`[SYNC] Removed remote event ${commitId} because no local mirror remains.`);
                }
            }
        }
    }
    countPendingLocalEvents(eventsDir) {
        if (!fs.existsSync(eventsDir)) {
            return 0;
        }
        return fs.readdirSync(eventsDir).filter(file => file.endsWith('.json')).length;
    }
    async getWorkingTreeChanges(repoPath) {
        const { exec } = await Promise.resolve().then(() => require('child_process'));
        const { promisify } = await Promise.resolve().then(() => require('util'));
        const execAsync = promisify(exec);
        try {
            ...
            extension_1.logger.appendLine(`[STATUS] Could not inspect the current working tree for ${repoPath}: ${error}`);
            return [];
        }
    }
    dispose() {
